...
class Game {
    ...
    _postMoveHandler(prevState) {
        if (JSON.stringify(this.board) !== prevState) {
            ...
            else if (!this.hasAvailableMoves()) this.status = "lose";
    ...
    hasAvailableMoves() {
        ...
        for(let i = 0; i < this.board.length - 1; i++)for(let j = 0; j < this.board[i].length; j++){
    ...
    addRandomTile() {
        ...
        for(let r = 0; r < this.board.length; r++){
            for(let c = 0; c < this.board[r].length; c++)if (this.board[r][c] === 0) emptyTiles.push({
                row: r,
                col: c
            });
        }
